...
export default function Wizard({ unitList }: WizardProps) {
  ...
  function ReviewRow({ label, value }: { label: string; value: React.ReactNode }) {
    return (
      <div className="flex justify-between items-center py-0.5 border-b border-dashed border-slate-100 last:border-0">
        ...
        {files.length > 0 && (
          <div className="space-y-2">
            {files.map((file, idx) => (
              ...
              >
                {isImage ? (
                  <img src={URL.createObjectURL(file)} alt={file.name} className="w-16 h-16 object-cover rounded shadow-sm" />
                ) : (
