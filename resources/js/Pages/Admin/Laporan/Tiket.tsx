...
export default function LaporanTiket({ 
    ...
    const getStatusStyle = (status: string) => {
        const map: Record<string, any> = {
            'open': { text: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-100 dark:bg-blue-500/20', dot: 'bg-blue-500' },
            'on_proses': { text: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-100 dark:bg-orange-500/20', dot: 'bg-orange-500' },
            'pending': { text: 'text-zinc-600 dark:text-zinc-400', bg: 'bg-zinc-100 dark:bg-zinc-500/20', dot: 'bg-zinc-500' },
            'waiting_approval': { text: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-100 dark:bg-purple-500/20', dot: 'bg-purple-500' },
            'need_revision': { text: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-100 dark:bg-amber-500/20', dot: 'bg-amber-500' },
            'solve': { text: 'text-green-600 dark:text-green-400', bg: 'bg-green-100 dark:bg-green-500/20', dot: 'bg-green-500' },
            'reject': { text: 'text-red-600 dark:text-red-400', bg: 'bg-red-100 dark:bg-red-500/20', dot: 'bg-red-500' },
            'dibatalkan': { text: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-100 dark:bg-rose-500/20', dot: 'bg-rose-500' },
        };
        return map[status] || { text: 'text-slate-600 dark:text-slate-400', bg: 'bg-slate-100 dark:bg-slate-500/20', dot: 'bg-slate-500' };
    };
    ...
    return (
        <AdminLayout title="Laporan Tiket">
            ...
            <div className="space-y-6">
                ...
                {/* Filter Bar */}
                {showFilter && (
                    <Card className="bg-slate-50/50 dark:bg-zinc-900/50 border-dashed print:hidden">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <Card className="flex flex-col print:break-inside-avoid">
                        ...
                        <CardContent className="flex-1 flex flex-col justify-center">
                            ...
                            <div className="rounded-lg border bg-slate-50 p-3 text-center dark:bg-zinc-900">
                ...
                <div className="grid gap-4 grid-cols-1">
                    <Card className="print:break-inside-avoid shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <CardContent className="p-0">
                            ...
                            <div className="overflow-x-auto max-h-[350px] relative scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-zinc-700 scrollbar-track-transparent hover:scrollbar-thumb-slate-300 dark:hover:scrollbar-thumb-zinc-600 transition-all">
                                <table className="w-full text-sm text-left">
                                    <thead className="sticky top-0 bg-slate-50/95 dark:bg-zinc-900/95 backdrop-blur-sm z-10">
                                        <tr>
                                            <th className="py-3 px-4 font-semibold text-slate-600 dark:text-slate-300">Jenis Layanan</th>
                                            <th className="py-3 px-4 font-semibold text-right text-slate-600 dark:text-slate-300 w-1/3 md:w-1/4">Distribusi Tiket</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                                        {ticketsByLayanan?.map((t: any, idx: number) => {
                                            ...
                                            return (
                                                <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-zinc-900/50 transition-colors group">
                                                    <td className="py-3 px-4 font-medium">{t.layanan}</td>
                                                    <td className="py-3 px-4">
                                                        <div className="flex items-center justify-end gap-3">
                                                            <div className="flex-1 h-2 bg-slate-100 dark:bg-zinc-800 rounded-full overflow-hidden max-w-[200px]">
                                                                <div 
                                                                    className="h-full bg-blue-500 dark:bg-blue-600 rounded-full group-hover:bg-blue-400 transition-all duration-500 ease-out" 
                                                                    style={{ width: `${percent}%` }}
                                                                ></div>
                                                            </div>
